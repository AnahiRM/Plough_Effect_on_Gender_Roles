...
data <- data[ ,c("female_ownership","plow", "agricultural_suitability", "tropical_climate", "large_animals", "political_hierarchies", "economic_complexity", "ln_income", "ln_income_squared")] 
...
r_3 <- foreach(k = 1:split, .combine='rbind', .inorder=FALSE, .packages=c('MASS','randomForest','neuralnet','gbm', 'sandwich', 'hdm', 'nnet', 'rpart','glmnet')) %dopar% { 
  set.seed(k)
...
save(r_3, data_info, file = file.path("outputs", "plough_t4_c3_output.RData"))
...
for (i in 1:(length(methods)+1)) {
  result[1,i] = median(r_3[,i])
  result[2,i] = median(r_3[,i+7])
}
print(xtable(result, type="latex", digits=3), file = file.path("outputs", "PL_T1_PB.txt"))
