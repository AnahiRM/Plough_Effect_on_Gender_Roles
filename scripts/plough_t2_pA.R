...
data <- data[ ,c("flfp2000","plow", "agricultural_suitability", "tropical_climate", "large_animals", 
                 "political_hierarchies", "economic_complexity", "ln_income", "ln_income_squared", 
                 "intensity_agriculture", "husbandry", "hunting", "abs_inherit",
                 "matrilocal", "patrilocal", "nuclear_fam", "extended_fam", "year_obs",
                 "years_civil_conflict", "years_interstate_conflict", "rugged", "communist_dummy",
                 "european_descent", "oil_pc", "agr_va_gdp2000", "man_va_gdp2000", "serv_va_gdp2000",
                 "cath00", "prot00", "othchrist00", "muslim00", "hindu00", 
...
r_2 <- foreach(k = 1:split, .combine='rbind', .inorder=FALSE, .packages=c('MASS','randomForest','neuralnet','gbm', 'sandwich', 'hdm', 'nnet', 'rpart','glmnet')) %dopar% { 
  set.seed(k)
...
save(r_2, data_info, file = file.path("outputs", "plough_t2_pA_output.RData"))
...
for (i in 1:(length(methods)+1)) {
  result[1,i] = median(r_2[,i])
  result[2,i] = median(r_2[,i+7])
}
print(xtable(result, type="latex", digits=3), file = file.path("outputs", "PL_T2_pA.txt"))
