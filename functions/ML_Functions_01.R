...
#  Modified: Neural Net: Not Scaling columns that contain only the same value which can happen when     #
#  splitting fixed effects with only a few observations                                                 #
...
rlassoF <- function(datause, dataout, form_x, form_y, post, logit=FALSE, arg){
  ...
  if(logit==FALSE){
   ...
   #print(lasso$coefficients[lasso$coefficients !=0])
  }
...
nnetF <- function(datause, dataout, form_x, form_y, clas=FALSE, arg){
  ...
  # datause[sapply(sapply(datause,unique), length) ==2] <- lapply(datause[sapply(sapply(datause,unique), length) ==2], as.factor)
  # dataout[sapply(sapply(dataout,unique), length) ==2] <- lapply(dataout[sapply(sapply(dataout,unique), length) ==2], as.factor)
  linout=FALSE
...
error <- function(yhat, y){
  ...
  # FORCE ATOMIC TYPE: Use unlist and as.numeric to stop the "list to double" error
  #yhat_clean <- as.numeric(unlist(yhat))
  #y_clean    <- as.numeric(unlist(y))
  
  # Standard MSE calculation
  #err <- sqrt(mean((yhat_clean - y_clean)^2))
  
  # Standard misclassification calculation
  #mis <- sum(abs(as.numeric(yhat_clean > .5) - (as.numeric(y_clean)))) / length(y_clean)
